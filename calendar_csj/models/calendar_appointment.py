...
class CalendarClass(models.Model):
    ...
    type = fields.Selection(
        [("audience", "Audience"), ("other", "Other")], default="audience"
    )
...
class CalendarAppointment(models.Model):
    ...
    state = fields.Selection(
        [
            ...
            ("assist_cancel", "Assisted and Canceled"),
            ("cancel", "Canceled"),
        ],
        "State",
        default="draft",
    ...
    appointment_code = fields.Char(
        string="Document Code", readonly=True, required=False, copy=False
    )
    ...
    class_id = fields.Many2one(
        "calendar.class", "Calendar class", ondelete="set null"
    )  # Clase
    help_id = fields.Many2one(
        "calendar.help", "Calendar help", ondelete="set null"
    )  # Ayuda
    partaker_type = fields.Many2one(
        "calendar.help", "Portaker Type", ondelete="set null"
    )  # Ayuda
    connection_type = fields.Many2one(
        "calendar.help", "Connection Type", ondelete="set null"
    )  # Ayuda
    request_date = fields.Date("Request date")  # Date
    appointment_date = fields.Date(
        "Appointment date", default=fields.Date.today()
    )  # Date
    appointment_close_date = fields.Date("Close date")
    appointment_close_user_id = fields.Many2one("res.users", "Close user")  # Date
    appointment_close_user_login = fields.Char(
        "Closing User Login", related="appointment_close_user_id.login", store=False
    )  # Date
    calendar_type = fields.Selection(
        [("unique", "Unique"), ("multi", "Multi")], "Calendar type", default="unique"
    )  # Agenda
    calendar_datetime = fields.Datetime(
        "Calendar datetime", tracking=True
    )  # Fechatag_number
    ...
    applicant_id = fields.Many2one(
        "res.partner", "Applicant", ondelete="set null"
    )  # Solicitante
    declarant_id = fields.Many2one(
        "res.partner", "Declarant", ondelete="set null"
    )  # Declarante
    declarant_text = fields.Text("Declarant input")
    indicted_id = fields.Many2one(
        "res.partner", "Indicted", ondelete="set null"
    )  # Procesado
    indicted_text = fields.Text("Indicted input")
    applicant_id_label = fields.Char(
        "Applicant Label", compute="_get_applicant_id_label", store=True
    )
    ...
    applicant_email = fields.Char(
        ...
        inverse="_inverse_applicant_id",
    )
    applicant_domain = fields.Char(
        "Applicant domain", compute="_compute_applicant_domain"
    )
    ...
    city_id = fields.Many2one(
        "res.city", "City", ondelete="set null", related="partner_id.city_id"
    )
    country_state_id = fields.Many2one(
        "res.country.state", "Country State", related="city_id.state_id"
    )
    ...
    judged_only_code = fields.Char(
        "Partner Only Code", compute="_compute_partner_separated_name", store=False
    )
    judged_only_name = fields.Char(
        "Partner Only Name", compute="_compute_partner_separated_name", store=True
    )
    ...
    partners_ids = fields.Many2many(
        "res.partner",
        "appointment_partner_rel",
        "appointment_id",
        "partner_id",
        "Partners",
    )
    partner_ids_label = fields.Char(
        "Partners Label", compute="_get_partner_ids_label", store=True
    )
    destination_ids = fields.Many2many(
        "res.partner",
        "appointment_destination_partner_rel",
        "appointment_id",
        "partner_id",
        "Destinations",
    )
    destination_ids_label = fields.Char(
        "Detinations Label", compute="_get_destination_ids_label", store=True
    )
    request_type = fields.Selection(
        [("l", "Free"), ("r", "Reserved")], "Request type", default="r"
    )
    request_type_label = fields.Char(
        "Request Type Label", compute="_get_request_type_label", store=False
    )
    ...
    reception_id = fields.Many2one(
        "calendar.reception", "Reception medium", ondelete="set null"
    )
    ...
    appointment_type_id = fields.Many2one(
        "calendar.appointment.type", "Online Appointment", default=_default_type_id
    )
    ...
    create_uid_login = fields.Char(
        "Create User Login", related="create_uid.login", store=False
    )
    ...
    def _compute_partner_separated_name(self):
        for record in self:
            code_entity = (
                record.partner_id.entity_id.code if record.partner_id.entity_id else ""
            )
            code_specialty = (
                record.partner_id.specialty_id.code
                if record.partner_id.specialty_id
                else ""
            )
            zipcode = (
                record.partner_id.city_id.zipcode if record.partner_id.city_id else ""
            )
    ...
    def _compute_record_data(self):
        for record in self:
            if record.calendar_datetime:
                tz_offset = (
                    self.env.user.tz_offset if self.env.user.tz_offset else False
                )
                tz = int(tz_offset) / 100 if tz_offset else 0
                date = (
                    record.calendar_datetime + datetime.timedelta(hours=tz)
                    if record.calendar_datetime
                    else False
                )
    ...
    def _compute_tag_number(self):
        for record in self:
            if (
                record.city_id
                and record.city_id.zipcode
                and (record.room_id or record.type != "audience")
                and record.process_number
    ...
    def _compute_calendar_datetime(self):
        for record in self:
            ...
            record.calendar_date = (
                record.calendar_datetime.date() if record.calendar_datetime else False
            )
            tz_offset = self.env.user.tz_offset if self.env.user.tz_offset else False
            tz = int(tz_offset) / 100 if tz_offset else 0
            record.calendar_time = (
                record.calendar_datetime.hour
                + tz
                + record.calendar_datetime.minute / 60.0
                if record.calendar_datetime
                else False
            )
    ...
    def _compute_applicant_id(self):
        for record in self:
            record.applicant_email = (
                record.applicant_id.email if record.applicant_id else ""
            )
            record.applicant_mobile = (
                record.applicant_id.mobile if record.applicant_id else ""
            )
...
>>>>>>> a95cd8a6880eaa5a4cce3ba508de54f9f33deb72
    ...
    def write(self, vals):
        if vals.get("calendar_datetime"):
            ...
            vals["sequence_icsfile_ctl"] = (
                self.sequence_icsfile_ctl + 1 if int(self.sequence_icsfile_ctl) else 1
            )
    ...
    def create_lifesize(self, vals):
        api = {
            ...
            "ownerExtension": self.env.user.extension_lifesize
            or self.env.user.company_id.owner_extension,
            "hiddenMeeting": "true",
            ...
            api.update(
                {
                    "ownerExtension": self.env.user.extension_lifesize
                    or self.env.user.company_id.owner_extension,
                    "moderatorExtension": judged_extension_lifesize
                    or self.env.user.extension_lifesize
                    or self.env.user.company_id.owner_extension,
                }
            )
        else:
    ...
    def create_event(self, vals):
        ...
        if not flag:
            type_id = self.env["calendar.appointment.type"].browse(
                vals.get("appointment_type_id")
            )
        ...
        try:
            for index, val in enumerate(res["datas"]):
                if fields_name.get("calendar_time"):
                    ...
                    res["datas"][index][fieldindex] = "{0:02d}:{1:02d}:00".format(
                        hour, minute
                    )
            ...
            raise UserError(
                "It was not possible to convert the time format when exporting the file."
            )
        return res
    ...
    def fetch_calendar_verify_availability(
        ...
        calendar_appointment_type_obj = self.env["calendar.appointment.type"].browse(
            int(calendar_appointment_type_id)
        )
        if not calendar_appointment_type_obj.judged_id.calendar_verify_availability(
            date_start, date_end
        ):
            return False
        return True
...
class CalendarAppointmentType(models.Model):
    ...
    city_id = fields.Many2one(
        "res.city", string="City", related="judged_id.city_id", store=True
    )
    type = fields.Selection(
        [
            ("audience", "Audience"),
    ...
    def search_calendar(self, judged_id):
        res = self.env["calendar.appointment.type"].search(
            [("judged_id", "=", judged_id)]
        )
        return res
